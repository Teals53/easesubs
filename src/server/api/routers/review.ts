...
export const reviewRouter = createTRPCRouter({
  ...
  canReview: protectedProcedure
    ...
    .query(async ({ ctx, input }) => {
      ...
      if (!orderItem) {
        return { canReview: false, reason: "Order item not found or order not completed" };
      }

      if (orderItem.reviews.length > 0) {
        return { canReview: false, reason: "Already reviewed", existingReview: orderItem.reviews[0] };
      }
