...
export async function POST(request: NextRequest) {
  try {
    ...
    const updatedPayment = await db.$transaction(async (tx) => {
      ...
      if (paymentStatus === "COMPLETED") {
        ...
        if (stockValidationErrors.length > 0) {
          console.warn("⚠️ Stock validation failed during payment completion:", {
          ...
          const planIds = payment.order.items.map((item: typeof payment.order.items[0]) => item.planId);
    ...
    if (paymentStatus === "COMPLETED" && payment.order.user.email) {
      try {
        const orderItems = payment.order.items.map((item: typeof payment.order.items[0]) => ({
          productName: item.plan.product.name,
      ...
      try {
        for (const item of payment.order.items) {
          try {
            ...
            console.log(`✅ Processed delivery for item ${item.id}:`, deliveryResult);
          } catch (deliveryError) {
            console.error(`Failed to process delivery for item ${item.id}:`, deliveryError);
        ...
        try {
          const automaticPlanIds = payment.order.items
            .filter((item: typeof payment.order.items[0]) => item.plan.deliveryType === "AUTOMATIC")
          ...
          if (automaticPlanIds.length > 0) {
            ...
            for (const order of conflictingOrders) {
              ...
              if (hasStockConflict.some(Boolean)) {
                ...
                cancelledCount++;
                console.log(`🚫 Cancelled conflicting order ${order.orderNumber}`);
              }
            }
