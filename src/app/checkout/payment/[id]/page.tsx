...
import { Loader2, CreditCard, ArrowLeft, CheckCircle, XCircle, ExternalLink } from 'lucide-react'
import { trpc } from '@/lib/trpc'

type PaymentStatus = 'loading' | 'processing' | 'success' | 'error' | 'redirect'
...
export default function PaymentPage() {
  ...
  const { data: payment, isLoading: paymentLoading, error: paymentError, refetch } = trpc.payment.getPayment.useQuery(
    { paymentId },
    { 
  ...
  if (status === 'loading' || paymentLoading || (paymentStatus === 'loading' && retryCount > 0)) {
    return (
      <div className="min-h-screen bg-gradient-to-br from-gray-900 via-purple-900 to-violet-900 flex items-center justify-center">
        ...
        >
          ...
          <h2 className="text-xl font-semibold text-white mb-2">Loading Payment</h2>
          <p className="text-gray-400">
            {retryCount > 0 ? `Retrying... (${retryCount}/3)` : 'Please wait while we prepare your payment...'}
          </p>
        </motion.div>
      </div>
    )
    ...
    <div className="min-h-screen bg-gradient-to-br from-gray-900 via-purple-900 to-violet-900 flex items-center justify-center p-4">
      ...
      >
        {paymentStatus === 'loading' && (
          <>
            <Loader2 className="h-12 w-12 animate-spin text-purple-400 mx-auto mb-4" />
            <h2 className="text-xl font-semibold text-white mb-2">Preparing Payment</h2>
          ...
          <>
            ...
            {payment && (
              <div className="bg-gray-700/50 rounded-lg p-4 mb-6 text-left">
                ...
                <div className="space-y-1 text-sm text-gray-300">
                  <div>Payment ID: <span className="text-white font-mono text-xs">{payment.id}</span></div>
                  <div>Amount: <span className="text-white">{payment.currency} {Number(payment.amount).toFixed(2)}</span></div>
                  <div>Method: <span className="text-white">{payment.method}</span></div>
                  <div>Status: <span className="text-blue-400">{payment.status}</span></div>
                </div>
              </div>
            )}
            <div className="space-y-3">
              <motion.button
                ...
                whileTap={{ scale: 0.95 }}
                onClick={() => router.push(`/dashboard/orders/${payment?.orderId}`)}
