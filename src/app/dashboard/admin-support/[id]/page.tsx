...
export default function AdminTicketDetailPage() {
  ...
  const { data: ticket, isLoading, refetch } = trpc.admin.getTicketById.useQuery(
    { id: ticketId },
    { 
      ...
      >
        <div className="flex flex-col lg:flex-row lg:items-start justify-between gap-4 mb-6">
          <div className="flex-1 min-w-0">
            <h1 className="text-xl md:text-2xl font-bold text-white mb-2 break-words">{ticket.title}</h1>
            <p className="text-gray-400 mb-4 break-words">{ticket.description}</p>
            
            <div className="flex flex-wrap items-center gap-2 md:gap-4 text-sm">
              <div className="flex items-center gap-2">
                <span className="text-gray-400">ID:</span>
                <span className="text-white font-mono text-xs md:text-sm">#{ticket.id.slice(-8)}</span>
              </div>
              
              <div className="flex items-center gap-2">
                <span className="text-gray-400">Category:</span>
                <span className="text-white text-xs md:text-sm">{ticket.category.replace('_', ' ')}</span>
              </div>
              ...
              <div className="flex items-center gap-2">
                ...
                <span className="text-white text-xs md:text-sm">{formatDate(ticket.createdAt)}</span>
              </div>
            </div>
          </div>
          ...
          <div className="flex flex-col lg:items-end gap-4 lg:ml-4">
            ...
            <div className="flex items-center gap-2">
              {editingStatus ? (
                ...
                <div className="flex items-center gap-2">
                  <span className={`inline-flex items-center gap-1 px-3 py-1 rounded-full text-sm font-medium border ${getStatusColor(ticket.status)}`}>
                ...
                <div key={message.id} className={`flex gap-4 ${isCurrentUser ? 'flex-row-reverse' : 'flex-row'}`}>
                  <div className={`w-10 h-10 rounded-full flex items-center justify-center flex-shrink-0 shadow-lg ${
                    ...
                    <span className="text-white text-sm font-bold">
                      {message.user?.name?.[0]?.toUpperCase() || message.user?.email?.[0]?.toUpperCase() || 'U'}
                    </span>
                  </div>

                  <div className={`flex-1 max-w-2xl ${isCurrentUser ? 'text-right' : 'text-left'}`}>
          ...
          <form onSubmit={handleAddMessage} className="space-y-4">
            <div className="flex items-center gap-2 mb-3">
              <label className="flex items-center gap-2 text-sm text-gray-400">
                ...
                />
                <span className="hidden sm:inline">Internal message (not visible to customer)</span>
            ...
            <div className="relative">
              ...
              <div className="absolute bottom-3 right-3">
                ...
                >
                  ...
                  <span className="hidden sm:inline">{addMessageMutation.isPending ? 'Sending...' : 'Send'}</span>
                </button>
              </div>
            </div>
          </form>
