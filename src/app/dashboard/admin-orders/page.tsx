...
export default function AdminOrdersPage() {
  ...
  const { data: orders, isLoading, refetch } = trpc.admin.getOrders.useQuery({
    status: statusFilter === 'all' || statusFilter === '' ? undefined : statusFilter as "PENDING" | "PROCESSING" | "COMPLETED" | "CANCELLED",
  ...
  const handleStatusUpdate = async (orderId: string, status: string) => {
    try {
      await updateOrderMutation.mutateAsync({
        orderId,
        status: status as "PENDING" | "PROCESSING" | "COMPLETED" | "CANCELLED" | "FAILED"
      })
    } catch (error) {
      console.error('Failed to update order status:', error)
    }
                  ...
                  <tr key={order.id} className="border-b border-gray-700 hover:bg-gray-700/30 transition-colors">
                      ...
                      <div className="flex items-center space-x-3">
                        <div className="w-8 h-8 bg-gradient-to-br from-purple-500 to-blue-500 rounded-full flex items-center justify-center">
                          <span className="text-white font-bold text-xs">
                            {order.user?.name?.[0]?.toUpperCase() || order.user?.email?.[0]?.toUpperCase() || 'U'}
                          </span>
                        </div>
                        <div>
                          <p className="text-white font-medium">{order.user?.name || 'Anonymous'}</p>
                    ...
                    <td className="py-4 px-6">
                      <div className="flex items-center">
                        ...
                        <span className="text-white font-semibold">{formatCurrency(Number(order.total))}</span>
                      </div>
                    </td>
                    <td className="py-4 px-6">
        ...
        {orders && totalPages > 1 && (
          <div className="px-6 py-4 border-t border-gray-700 flex items-center justify-between">
            <div className="text-sm text-gray-400">
              Showing {(page - 1) * itemsPerPage + 1} to {Math.min(page * itemsPerPage, orders.total)} of {orders.total} orders
            </div>
            <div className="flex items-center space-x-2">
              <button
