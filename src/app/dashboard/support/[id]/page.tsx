...
export default function TicketDetailPage() {
  ...
  const { data: ticket, isLoading, refetch, isFetching } = trpc.ticket.getById.useQuery(
    { id: ticketId },
    { 
      enabled: !!ticketId,
  ...
  const canCloseTicket = ticket && !['CLOSED', 'RESOLVED'].includes(ticket.status)
  const canReopenTicket = ticket && ['CLOSED', 'RESOLVED'].includes(ticket.status)
    ...
    <div className="space-y-6">
      ...
      >
        <div className="flex items-start justify-between mb-6">
          <div className="flex-1">
            ...
            <div className="flex flex-wrap items-center gap-4 text-sm">
              <span className="text-gray-400">Ticket #{ticket.id.slice(-8)}</span>
              <span className="text-gray-600">•</span>
              <span className="text-gray-400">Created {formatDate(new Date(ticket.createdAt))}</span>
              <span className="text-gray-600">•</span>
              <span className={`font-medium ${getPriorityColor(ticket.priority)}`}>
                {ticket.priority} Priority
              </span>
              <span className="text-gray-600">•</span>
              <span className="text-gray-400">{ticket.category.replace('_', ' ')}</span>
            </div>
          </div>
          
          <div className="flex items-center gap-3">
            <span className={`inline-flex items-center gap-2 px-3 py-1 rounded-full text-sm font-medium border ${getStatusColor(ticket.status)}`}>
        ...
        <div className="bg-gray-700/30 rounded-lg p-4">
          ...
          <p className="text-gray-200 whitespace-pre-wrap leading-relaxed">{ticket.description}</p>
        </div>
      </motion.div>
            ...
            ticket.messages.map((message: TicketMessage) => (
              <div
                ...
                className={`flex gap-4 ${
                  message.user?.role === 'ADMIN' ? 'flex-row-reverse' : 'flex-row'
                }`}
              >
                ...
                <div className={`flex-1 max-w-md ${
                  ...
                  <div className={`mt-2 text-xs text-gray-500 ${
                    ...
                    <span className="font-medium">
                      {message.user?.role === 'ADMIN' ? 'Support Team' : (message.user?.name || 'You')}
                    </span>
        ...
        {!canAddMessage && (
          <div className="p-6 border-t border-gray-700 bg-gray-700/30">
            <p className="text-gray-400 text-center">
              This ticket is closed. {canReopenTicket && 'You can reopen it to continue the conversation.'}
            </p>
          </div>
        )}
      </motion.div>
