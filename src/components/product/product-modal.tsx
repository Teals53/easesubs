...
interface PlanData {
  ...
  billingPeriod: 'MONTHLY' | 'YEARLY' | 'LIFETIME' | 'CUSTOM'
  duration: number
  features: string[]
  isPopular: boolean
  isAvailable: boolean
...
export function ProductModal({ isOpen, onClose, onSuccess, product }: ProductModalProps) {
  ...
  const [formData, setFormData] = useState({
    ...
    slug: '',
    description: '',
    category: 'STREAMING_MEDIA' as 'STREAMING_MEDIA' | 'PRODUCTIVITY_TOOLS' | 'CREATIVE_DESIGN' | 'LEARNING_EDUCATION' | 'SOCIAL_COMMUNICATION' | 'GAMING' | 'BUSINESS_FINANCE' | 'HEALTH_FITNESS',
    logoUrl: '',
    logoName: '',
    borderColor: '',
    isActive: true,
  ...
  useEffect(() => {
    ...
    if (product?.id && fullProductData) {
      setFormData({
        ...
        category: (fullProductData.category || 'STREAMING_MEDIA') as 'STREAMING_MEDIA' | 'PRODUCTIVITY_TOOLS' | 'CREATIVE_DESIGN' | 'LEARNING_EDUCATION' | 'SOCIAL_COMMUNICATION' | 'GAMING' | 'BUSINESS_FINANCE' | 'HEALTH_FITNESS',
      ...
      if (fullProductData.plans && fullProductData.plans.length > 0) {
        ...
        setPlans(fullProductData.plans.map((plan: any) => ({
          id: plan.id,
          name: plan.name,
          planType: plan.planType,
          price: Number(plan.price.toString()),
          originalPrice: plan.originalPrice ? Number(plan.originalPrice.toString()) : undefined,
  ...
  const handlePlanChange = (index: number, field: string, value: string | number | boolean | undefined | null) => {
            ...
            <form onSubmit={handleSubmit} className="p-6 space-y-6">
              ...
              <div className="space-y-4">
                ...
                <div>
                  ...
                  />
                  <p className="text-xs text-gray-400 mt-1">
                    URL to the product logo image. If left empty, a default logo will be generated.
                  </p>
                </div>

                {/* SEO Section */}
                <div className="space-y-4">
                  <h4 className="text-md font-medium text-gray-200">SEO Settings</h4>
                  ...
                  <div>
                    ...
                    />
                    <p className="text-xs text-gray-400 mt-1">
                      Meta description for search engines. Keep it under 155 characters for best results.
                    </p>
                  </div>
                </div>
              ...
              <div className="space-y-4">
                <div className="flex items-center justify-between">
                  <h3 className="text-lg font-medium text-white">Pricing Plans</h3>
                  <button
                    type="button"
                    onClick={addPlan}
                ...
                {plans.map((plan, planIndex) => (
                  <div key={planIndex} className="bg-gray-700/50 p-4 rounded-lg border border-gray-600">
                    <div className="flex items-center justify-between mb-4">
                      <h4 className="text-md font-medium text-white">Plan {planIndex + 1}</h4>
                      {plans.length > 1 && (
                        <button
                          type="button"
                    ...
                    <div className="grid grid-cols-1 md:grid-cols-2 gap-4 mb-4">
                      ...
                      <div>
                        ...
                        <input
                          type="text"
                          value={plan.planType}
                          onChange={(e) => handlePlanChange(planIndex, 'planType', e.target.value)}
                          className="w-full px-3 py-2 bg-gray-700 border border-gray-600 rounded-lg text-white placeholder-gray-400 focus:outline-none focus:ring-2 focus:ring-purple-500"
                      ...
                      <div>
                        ...
                        <input
                          ...
                          onChange={(e) => handlePlanChange(planIndex, 'originalPrice', e.target.value ? parseFloat(e.target.value) : undefined)}
                          className="w-full px-3 py-2 bg-gray-700 border border-gray-600 rounded-lg text-white placeholder-gray-400 focus:outline-none focus:ring-2 focus:ring-purple-500"
                          placeholder="19.99"
                        />
                      </div>
                      ...
                      <div>
                        ...
                        <select
                          ...
                          onChange={(e) => handlePlanChange(planIndex, 'billingPeriod', e.target.value)}
                          className="w-full px-3 py-2 bg-gray-700 border border-gray-600 rounded-lg text-white focus:outline-none focus:ring-2 focus:ring-purple-500"
                          required
                        >
                      ...
                      <div>
                        ...
                        <input
                          type="number"
                          value={plan.duration}
                          onChange={(e) => handlePlanChange(planIndex, 'duration', parseInt(e.target.value) || 0)}
                          className="w-full px-3 py-2 bg-gray-700 border border-gray-600 rounded-lg text-white placeholder-gray-400 focus:outline-none focus:ring-2 focus:ring-purple-500"
                          placeholder="30"
                          required
                        />
                    ...
                    <div className="mb-4">
                      <h5 className="text-sm font-medium text-gray-300 mb-3">Stock Management</h5>
                      <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                        <div>
                          <label className="flex items-center space-x-2 mb-2">
                            <input
                              type="checkbox"
                              checked={plan.stockQuantity === null || plan.stockQuantity === undefined}
                              onChange={(e) => {
                                if (e.target.checked) {
                                  handlePlanChange(planIndex, 'stockQuantity', null)
                                } else {
                                  handlePlanChange(planIndex, 'stockQuantity', 100)
                                }
                              }}
                              className="w-4 h-4 text-purple-600 bg-gray-700 border-gray-600 rounded"
                            />
                            <span className="text-sm text-gray-300">Infinite Stock</span>
                          </label>
                          {plan.stockQuantity !== null && plan.stockQuantity !== undefined && (
                            <div>
                              ...
                              <input
                                ...
                                onChange={(e) => handlePlanChange(planIndex, 'stockQuantity', parseInt(e.target.value) || 0)}
                                className="w-full px-3 py-2 bg-gray-700 border border-gray-600 rounded-lg text-white placeholder-gray-400 focus:outline-none focus:ring-2 focus:ring-purple-500"
                                placeholder="100"
                              />
                              <p className="text-xs text-gray-400 mt-1">
                        ...
                        <div>
                          ...
                          <input
                            ...
                            onChange={(e) => handlePlanChange(planIndex, 'maxSubscriptions', e.target.value ? parseInt(e.target.value) : undefined)}
                            className="w-full px-3 py-2 bg-gray-700 border border-gray-600 rounded-lg text-white placeholder-gray-400 focus:outline-none focus:ring-2 focus:ring-purple-500"
