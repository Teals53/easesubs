...
export function useCart() {
  ...
  const { items, setItems, isHydrated, getTotalItems, getTotalPrice, toggleCart } = useCartStore()
  ...
  useEffect(() => {
    if (serverCart?.items && isHydrated) {
      const formattedItems = serverCart.items.map(serverItem => ({
        ...
        originalPrice: serverItem.plan.originalPrice ? Number(serverItem.plan.originalPrice) : undefined,
