...
export class Cryptomus {
  ...
  private createSignature(data: Record<string, unknown> | object, apiKey: string): string {
  ...
  private async makeRequest<T>(
    ...
    if (!apiKey) {
      throw new Error(`${usePayoutKey ? 'Payout' : 'Payment'} API key not provided`)
    }
  ...
  // Payment methods
  async createPayment(data: CreatePaymentRequest): Promise<CryptomusResponse<PaymentResult>> {
    return this.makeRequest<PaymentResult>('/v1/payment', data)
  }

  async getPaymentInfo(data: PaymentInfoRequest): Promise<CryptomusResponse<PaymentResult>> {
    return this.makeRequest<PaymentResult>('/v1/payment/info', data)
  }

  async getPaymentHistory(data: PaymentHistoryRequest = {}): Promise<CryptomusResponse<PaymentHistoryResult>> {
  ...
  async resendWebhook(data: ResendWebhookRequest): Promise<CryptomusResponse<null>> {
    return this.makeRequest<null>('/v1/payment/resend', data)
  }

  async testWebhook(data: ResendWebhookRequest): Promise<CryptomusResponse<null>> {
    return this.makeRequest<null>('/v1/test-webhook/payment', data)
  }

  // Wallet methods
  async createWallet(data: CreateWalletRequest): Promise<CryptomusResponse<WalletResult>> {
    return this.makeRequest<WalletResult>('/v1/wallet', data)
  }

  async blockWallet(data: BlockWalletRequest): Promise<CryptomusResponse<BlockWalletResult>> {
    return this.makeRequest<BlockWalletResult>('/v1/wallet/block-address', data)
  }

  // Payout methods
  async createPayout(data: CreatePayoutRequest): Promise<CryptomusResponse<PayoutResult>> {
    return this.makeRequest<PayoutResult>('/v1/payout', data, true)
  }

  async getPayoutInfo(data: PayoutInfoRequest): Promise<CryptomusResponse<PayoutResult>> {
    return this.makeRequest<PayoutResult>('/v1/payout/info', data, true)
  }

  async getPayoutHistory(data: PayoutHistoryRequest = {}): Promise<CryptomusResponse<PayoutHistoryResult>> {
  ...
  // Webhook validation
  static validateWebhook(body: string, signature: string, apiKey: string): boolean {
    try {
      ...
      const { sign: _, ...dataWithoutSign } = webhookData
  ...
  async getExchangeRate(from: string, to: string, amount?: string): Promise<CryptomusResponse<ExchangeRateResult>> {
