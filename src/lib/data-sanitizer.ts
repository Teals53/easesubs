...
class DataSanitizer {
   ...
   */
  sanitizeUser(userData: Record<string, unknown>, isOwner = false): Record<string, unknown> {
    if (isOwner) {
      // Owner can see more fields but still sanitized
      const config = {
        ...USER_SANITIZATION_CONFIG,
        excludeFields: ['password', 'passwordHash', 'resetToken', 'verificationToken']
      };
   ...
   */
  sanitizePayment(paymentData: Record<string, unknown>): Record<string, unknown> {
   ...
   * Sanitize order data
   */
  sanitizeOrder(orderData: Record<string, unknown>, isOwner = false): Record<string, unknown> {
  ...
  private sanitizeData(
    ...
    for (const [key, value] of Object.entries(data)) {
      ...
      if (config.excludeFields.some(field => lowerKey.includes(field.toLowerCase()))) {
        continue;
      }
      ...
      if (config.maskFields.some(field => lowerKey.includes(field.toLowerCase()))) {
        sanitized[key] = this.maskValue(value);
        continue;
      }

      // Hash fields that need to be compared but not exposed
      if (config.hashFields.some(field => lowerKey.includes(field.toLowerCase()))) {
      ...
      if (value && typeof value === 'object') {
        if (Array.isArray(value)) {
          ...
          sanitized[key] = this.sanitizeData(value as Record<string, unknown>, config);
        }
      } else {
        sanitized[key] = value;
      }
   ...
   */
  validateNoSensitiveData(data: unknown): { isValid: boolean; violations: string[] } {
