...
export class PaymentProviders {
  ...
  static async createCryptomusPayment(data: CryptomusPaymentData): Promise<{
    ...
    try {
      ...
      if (!merchantId || !paymentApiKey || merchantId === 'your-merchant-uuid-here') {
        return {
          success: false,
      ...
      const paymentRequest: CreatePaymentRequest = {
        ...
        url_callback: data.callbackUrl || `${process.env.NEXTAUTH_URL}/api/webhooks/cryptomus`,
      ...
      if (response.state === 0 && response.result) { // Success
        return {
          success: true,
        ...
        throw new Error(response.message || 'Cryptomus payment creation failed')
      }
    } catch (error) {
  ...
  static async getCryptomusPaymentInfo(paymentId: string, orderId?: string): Promise<{
    success: boolean
    payment?: unknown
    error?: string
  }> {
  ...
  static async createCryptomusWallet(
    ...
    try {
      ...
      const response = await cryptomus.createWallet({
        ...
        currency,
        order_id: orderId,
        url_callback: callbackUrl || `${process.env.NEXTAUTH_URL}/api/webhooks/cryptomus`
      })
