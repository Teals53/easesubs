...
export function sanitizeHtml(
  ...
  const dangerousTags = ['script', 'object', 'embed', 'applet', 'meta', 'iframe', 'form', 'input', 'link', 'style'];
  ...
  const dangerousAttrs = ['onclick', 'onload', 'onerror', 'onmouseover', 'onfocus', 'onblur', 'onsubmit', 'onchange', 'javascript:', 'vbscript:', 'data:'];
...
export function sanitizeEmail(email: string): string {
  ...
  if (sanitized.includes('..') || (sanitized.includes('+') && sanitized.split('+').length > 2)) {
...
export function sanitizeUrl(url: string): string {
  ...
  try {
    ...
    // Check for suspicious patterns
    if (parsed.hostname.includes('localhost') && process.env.NODE_ENV === 'production') {
