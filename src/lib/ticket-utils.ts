...
export async function createAutoTicketForOrder(
  ...
  tx?: Omit<PrismaClient, "$connect" | "$disconnect" | "$on" | "$transaction" | "$use" | "$extends">, // Optional transaction context
  ...
  const supportTicket = await dbContext.supportTicket.create({
    data: {
      ...
      tags: [
        "auto-created", 
        "new-order", 
        ...(isAdminBypass ? ["admin-bypass", "test-order"] : ["payment-completed"]),
  ...
  console.log(`✅ Created automatic support ticket ${ticketNumber} for completed order ${order.orderNumber}${isAdminBypass ? ' (Admin Bypass)' : ''}`);
