...
export class DeliveryService {
   ...
   */
  private static async processAutomaticDelivery(orderItem: OrderItemWithRelations) {
    ...
    if (!stockItem) {
      ...
      throw new Error(`No stock available for automatic delivery of ${orderItem.plan.product.name} - ${orderItem.plan.planType}`);
    }
   ...
   */
  private static async processManualDelivery(orderItem: OrderItemWithRelations, customDescription?: string) {
    // Generate ticket number
    const ticketCount = await db.supportTicket.count();
    const ticketNumber = `DELIVERY-${String(ticketCount + 1).padStart(6, "0")}`;

    const description = customDescription || `Manual delivery request for ${orderItem.plan.product.name} - ${orderItem.plan.planType} Plan.
...
Please process the delivery for this customer.`;
  ...
  static async isDelivered(orderItemId: string): Promise<boolean> {
    const orderItem = await db.orderItem.findFirst({
      where: { id: orderItemId },
      select: { deliveredAt: true, deliveryType: true, stockItemId: true, ticketId: true },
    });
